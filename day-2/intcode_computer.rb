class IntcodeComputer
	...
	def process_opcode_at(position)
		...
		case op_code
		when 1
		  add_at(position + 1, position + 2, position + 3)
		  determine_next_instruction(position)
		when 2
		  multiply_at(position + 1, position + 2, position + 3)
		  determine_next_instruction(position)
		when 99
		 end_program
		else
	...
	def add_at(op_value_one, op_value_two, store_value)
		position_one 	= @array[op_value_one]
		position_two 	= @array[op_value_two]
		store_position 	= @array[store_value]
		store_position = @array[store_value]

		@array[store_position] = @array[position_one] + @array[position_two]
	end

	def multiply_at(op_value_one, op_value_two, store_value)
		position_one 	= @array[op_value_one]
		position_two 	= @array[op_value_two]
		store_position 	= @array[store_value]
